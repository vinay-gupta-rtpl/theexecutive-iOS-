...
//  iReachability
//
//  Created by Mradul Mathur on 31/01/17.
//  Copyright © 2017 Mradul Mathur. All rights reserved.
//
...
public extension Reachability {
    ...
    var description: String {
        ...
        let wWamFlag = isRunningOnDevice ? (isOnWWANFlagSet ? "W" : "-") : "X"
        ...
        let connectionOnFlag = isConnectionOnTrafficFlagSet ? "C" : "-"
        let demandFlag = isConnectionOnDemandFlagSet ? "D" : "-"
        let localAddress = isLocalAddressFlagSet ? "l" : "-"
        let directFlag = isDirectFlagSet ? "d" : "-"

        return "\(wWamFlag)\(reachableFlag) \(connectionRequired)\(transientConnection)\(interventionFlag)\(connectionOnFlag)\(demandFlag)\(localAddress)\(directFlag)"
    }
}
