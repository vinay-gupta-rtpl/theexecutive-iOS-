...
extension UIImageView {
    ...
    public class func sharedImageCache() -> AFImageCacheProtocol {
        struct Static {
            static var defaultImageCache: AFImageCache? = {
                NotificationCenter.default.addObserver(forName: NSNotification.Name.UIApplicationDidReceiveMemoryWarning, object: nil, queue: OperationQueue.main) { (_) -> Void in
    ...
    fileprivate var afRequestImageOperation:(operation: Operation?, request: URLRequest?) {
        get {
    ...
    public func setImageWithUrlRequest(_ request: URLRequest, placeHolderImage: UIImage? = nil,
        ...
        self.afRequestImageOperation = (BlockOperation(block: { () -> Void in
            var response: URLResponse?
            do {
                let data = try NSURLConnection.sendSynchronousRequest(request, returning: &response)
                DispatchQueue.main.async(execute: { () -> Void in
                    if request.url! == self.afRequestImageOperation.request?.url {
                        ...
                        self.afRequestImageOperation = (nil, nil)
                    }
                })
            } catch {
                if failure != nil {
        ...
        UIImageView.af_sharedImageRequestOperationQueue().addOperation(self.afRequestImageOperation.operation!)
    }
    
    fileprivate func cancelImageRequestOperation() {
        self.afRequestImageOperation.operation?.cancel()
        self.afRequestImageOperation = (nil, nil)
    }
}
